...
import com.nanored.vpn.AppConfig.LOOPBACK
import com.nanored.vpn.BuildConfig
import com.nanored.vpn.util.Utils.encode
import com.nanored.vpn.util.Utils.urlDecode
...
object HttpUtil {
    ...
    fun getUrlContentWithUserAgent(url: String?, userAgent: String?,  timeout: Int = 15000, httpPort: Int = 0): String {
        ...
        while (redirects++ < maxRedirects) {
            ...
            val finalUserAgent = if (userAgent.isNullOrBlank()) {
                "NanoredVPN/${BuildConfig.VERSION_NAME}"
            } else {
                userAgent
            }
    ...
    @Throws(IOException::class)
    fun getUrlContentWithHeaders(url: String?, userAgent: String?, timeout: Int = 15000, httpPort: Int = 0): UrlContentResult {
        var currentUrl = url
        var redirects = 0
        val maxRedirects = 3
        ...
        while (redirects++ < maxRedirects) {
            ...
            when (responseCode) {
                in 300..399 -> {
                    ...
                    val headerNames = listOf("subscription-userinfo", "content-disposition", "profile-title", "support-url", "profile-update-interval")
                    for (name in headerNames) {
                        conn.getHeaderField(name)?.let { headerMap[name] = it }
                    }
                    val body = conn.inputStream.use { it.bufferedReader().readText() }
                    return UrlContentResult(body, headerMap)
                } finally {
                    conn.disconnect()
                }
            }
