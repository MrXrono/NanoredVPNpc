package com.nanored.vpn.viewmodel
...
import com.nanored.vpn.dto.ServersCache
import com.nanored.vpn.dto.SubscriptionCache
import com.nanored.vpn.extension.serializable
...
import com.nanored.vpn.handler.SpeedtestManager
import com.nanored.vpn.util.CountryUtils
import com.nanored.vpn.util.MessageUtil
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    data class CountryInfo(val code: String, val name: String, val flag: String, val serverGuids: List<String>)
    val countries by lazy { MutableLiveData<List<CountryInfo>>() }
    val countryPingResults by lazy { MutableLiveData<Map<String, Long>>() }
    var selectedCountryCode: String? = MmkvManager.decodeSettingsString(AppConfig.CACHE_SELECTED_COUNTRY)
        set(value) {
            field = value
            MmkvManager.encodeSettings(AppConfig.CACHE_SELECTED_COUNTRY, value ?: "")
        }
...
//            }
    ...
    fun getSubscriptions(context: Context): List<GroupMapItem> {
        ...
        subscriptions.forEach { sub ->
            groups.add(GroupMapItem(id = sub.guid, remarks = "Список подключений"))
        }
        return groups
    }

    fun buildCountryList() {
        val allServers = MmkvManager.decodeServerList()
        val countryMap = mutableMapOf<String, MutableList<String>>()
        for (guid in allServers) {
            val profile = MmkvManager.decodeServerConfig(guid) ?: continue
            val code = CountryUtils.extractCountryCode(profile.remarks) ?: continue
            countryMap.getOrPut(code) { mutableListOf() }.add(guid)
        }
        val list = countryMap.map { (code, guids) ->
            CountryInfo(
                code = code,
                name = CountryUtils.getCountryName(code),
                flag = CountryUtils.getCountryFlag(code),
                serverGuids = guids
            )
        }.sortedByDescending { it.serverGuids.size }
        countries.value = list
    }

    fun findBestServerForCountry(code: String, callback: (String?) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            val countryInfo = countries.value?.find { it.code == code }
            if (countryInfo == null || countryInfo.serverGuids.isEmpty()) {
            ...
            for (guid in countryInfo.serverGuids) {
                val profile = MmkvManager.decodeServerConfig(guid) ?: continue
                val server = profile.server ?: continue
                val port = profile.serverPort?.toIntOrNull() ?: continue
                val time = SpeedtestManager.socketConnectTime(server, port)
                if (time in 1 until bestTime) {
                    bestTime = time
                    bestGuid = guid
                }
            }
            ...
            if (bestGuid == null) {
                bestGuid = countryInfo.serverGuids.firstOrNull()
            }

            withContext(Dispatchers.Main) { callback(bestGuid) }
        }
    }

    fun testAllCountryPings() {
        viewModelScope.launch(Dispatchers.IO) {
            val countryList = countries.value ?: return@launch
            val results = mutableMapOf<String, Long>()

            for (country in countryList) {
                var bestTime = Long.MAX_VALUE
                for (guid in country.serverGuids) {
                    val profile = MmkvManager.decodeServerConfig(guid) ?: continue
                    val server = profile.server ?: continue
                    val port = profile.serverPort?.toIntOrNull() ?: continue
                    val time = SpeedtestManager.socketConnectTime(server, port)
                ...
                if (bestTime < Long.MAX_VALUE) {
                    results[country.code] = bestTime
                }
                withContext(Dispatchers.Main) {
                    countryPingResults.value = HashMap(results)
                }
            }
        }
    }
